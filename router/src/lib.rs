...
pub static SNOW_WEATHER_ROUTER_WATERMARK: [u8; b"SnowWeatherRouter|Snownight|v32-touch-input\0".len()] =
    *b"SnowWeatherRouter|Snownight|v32-touch-input\0";
...
unsafe fn call_dispatch_pointer_data_packet(
    holder: *mut c_void,
    packet: *const u8,
    packet_len: usize,
) {
    ...
    type Function = unsafe extern "C" fn(*mut c_void, *const u8, usize);
    let function: Function = mem::transmute(interface_entry(0x68));
    function(holder, packet, packet_len);
}
...
unsafe extern "C" fn platform_message_callback(
    ...
    if bytes_equal(channel, CHANNEL_SYSTEM_BRIDGE) {
        match standard_method(payload) {
            ...
            Some(method) if method == b"get_files_dir" => reply(state, reply_id, STANDARD_FILES_DIR),
            _ => reply(state, reply_id, STANDARD_NULL),
        }
        return;
    }
...
unsafe fn append_pointer_data(
    ...
    // Flutter 3.10 PointerData is a 36-field, little-endian 64-bit record.
    ...
    packet.put_i64(0); // pointer_identifier, assigned by Flutter
    ...
    packet.put_i64(0); // view_id (zero for the main Flutter view)
}

unsafe fn dispatch_motion_event(state: *mut RouterState, event: *mut AInputEvent) {
    if (*state).holder.is_null() || event.is_null()
    ...
    let action_index =
        ((action >> AMOTION_EVENT_ACTION_POINTER_INDEX_SHIFT)
            & AMOTION_EVENT_ACTION_POINTER_INDEX_MASK) as usize;
    ...
    match masked_action {
        AMOTION_EVENT_ACTION_DOWN | AMOTION_EVENT_ACTION_POINTER_DOWN => {
            if action_index < pointer_count {
                append_pointer_data(event, action_index, 4, 0, &mut packet);
            }
        }
        AMOTION_EVENT_ACTION_MOVE => {
            for index in 0..pointer_count {
                append_pointer_data(event, index, 5, 0, &mut packet);
            }
        }
        AMOTION_EVENT_ACTION_UP | AMOTION_EVENT_ACTION_POINTER_UP => {
            for index in 0..pointer_count {
                if index != action_index {
                    append_pointer_data(event, index, 5, 1, &mut packet);
                }
            }
            if action_index < pointer_count {
                append_pointer_data(event, action_index, 6, 0, &mut packet);
            }
        }
        AMOTION_EVENT_ACTION_CANCEL => {
            for index in 0..pointer_count {
                append_pointer_data(event, index, 0, 0, &mut packet);
            }
        }
        _ => {}
    }
    ...
    if packet.len != 0 {
        call_dispatch_pointer_data_packet(
            (*state).holder,
            packet.bytes.as_ptr(),
            packet.len,
        );
...
unsafe extern "C" fn input_queue_callback(
    ...
    loop {
        ...
        // Keep consuming the queue so Android's InputDispatcher does not time out.
